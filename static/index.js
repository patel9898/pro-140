...
function toggleChatWindow() {
    ...
    // When the chatbot button is clicked
    $('#chatbot-button').click(function () {

        // Slide toggle the chat window
        $('.chatbox__chat').slideToggle()
    });
...
function askBot() {
    ...
    $("#chatbot-button").click(function () {
        ...
        // Get the text from the textbox in the chatbot
        var user_message = $("#bot_input_text").text().trim()
    
        // Check if the user message is not empty
        if (user_message != "") {
           
            // Replace the existing content of the chat window with a new div element that contains the user message
            $("#chat_messages").html('<div class="user__messages">' + user_message + ' </div>')
            
            // Clear the text input box after sending message
            $("#bot_input_text").text('');
    
            // Create an object with the user message as a property
            let chat_input_data = {
                "user_message": user_message
            }
    
            // Send the object to the server for processing (this part may vary depending on your server logic)
            $.ajax({
                type: "POST",
                url: "/process_message",
                data: chat_input_data,
                success: function(response) {
                    // Handle the response from the server (this part may vary depending on your server logic)
                }
            });
